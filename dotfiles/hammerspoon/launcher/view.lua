...
local function replaceOnce(document, marker, content)
	...
	if count ~= 1 then error("launcher document is missing " .. marker) end
	return updated
end

local function readDocument()
	local html = readFile(documentPath)
	html = replaceOnce(html, "__LAUNCHER_STYLE__", "<style>\n" .. readFile(stylePath) .. "\n</style>")
	html = replaceOnce(html, "__LAUNCHER_SCRIPT__", "<script>\n" .. readFile(scriptPath) .. "\n</script>")
	return html
end

local function emit(event, callback)
	...
	webview:evaluateJavaScript("window.plater.receive(" .. encoded .. ")", function(_, err)
		if err and err.code ~= 0 then hs.printf("launcher web UI: %s", hs.inspect(err)) end
		if callback then callback(err and err.code ~= 0 and err or nil) end
	end)
	return true
end
...
local function hide()
	...
	if webview then
		emit({ type = "launcher.closed", payload = {} })
		webview:hide()
	end
end
...
local function handleMessage(message)
	...
	if event.type == "launcher.rendered" and requested then
		webview:show():bringToFront(true)
		hs.timer.doAfter(0.02, function()
			if requested then emit({ type = "launcher.focus", payload = {} }) end
		end)
...
function View.isVisible()
	return requested or (webview and webview:isVisible()) or false
end

function View.reload()
	local ok, html = pcall(readDocument)
	if not ok then
		hs.printf("launcher reload failed; keeping current UI: %s", html)
		return false
	end
	ready = false
	allowBlurClose = false
